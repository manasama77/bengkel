...
<x-jstooltip/>
    ...
    <form id="setting-form" method="POST" action="{{route('admin.portofolio.update',$item->id)}}">
        @method('put')
        @csrf
    <div class="row py-2 px-2">
        ...
        <div class="form-group row align-items-center py-2">
            ...
            <div class="col-sm-6 col-md-9">
              ...
              <input type="text" class="form-control  @error('title') is-invalid @enderror" name="title" required  value="{{old('title')?old('title'):$item->title}}">
          ...
          <div class="form-group row align-items-center py-2">
              ...
              >{{old('desc')?old('desc'):$item->desc}}</textarea>
                ...
                <div class="col-sm-6 col-md-9">
                    ...
                    @enderror" name="kategori[]"  style="width: 100%" multiple="multiple" required>
                        ...
                        @foreach ($kategori as $t)
                            @php
                                $label=App\Models\label::where('parrent_id',$item->id)->where('nama',$t->nama)->count();
                            @endphp
                            <option value="{{ $t->id }}" {{$label>0?'selected':''}}> {{ $t->nama }}</option>
                        @endforeach
                      </select>
