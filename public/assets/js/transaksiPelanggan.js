function refreshDataRestok() {
    ...
    if (getData) {
        ...
        <button class="btn btn-danger btn-sm" onclick="return  confirm('Anda yakin menghapus data ini? Y/N') ?storeHapusData(${
            el.id
        }):''"><span
        class="pcoded-micon"> <i class="fa-solid fa-trash-can"></i></span></button>

        <button class="btn btn-warning btn-sm" onclick="storeBtnOpenModalEdit(${
            el.id
        },${index})" data-bs-toggle="modal" data-bs-target="#formModalEdit"><span
        ...
        let sumtotalbayar = getData
            .map((item) => item.total)
            .reduce((prev, next) => prev + next);
        let totalberat = getData
            .map((item) => item.berat * item.jumlah)
            .reduce((prev, next) => prev + next);
...
function storeBtnOpenModalEdit(id = null, index = null) {
    ...
    $("#formModalEdit").on("shown.bs.modal", function () {
        ...
        $("#inputHargaAsli").val(
            `${rupiah(getData[index].harga_asli)} / Stok : ${
                getData[index].stok
            }`
        );
...
//     await axios.get(`{{ env('APP_URL') }}/pelanggan/rajaongkir/cost?provinsi_id=${provinsi_id}&city_id=${city_id}&courir=${courir}&weight=${weight}`)
//         .then(response => {
...
<img src="${
                    datas[i].img
                }" class="thumbnail img-responsive"  style="display: block;max-width: 100%;height: 200px;object-fit: cover">
<div class="card-body">
<h5 class="card-title">${datas[i].nama}</h5>
<p class="card-text">Harga : Rp ${rupiah(datas[i].harga_jual)},00 - Stok : ${
                    datas[i].stoktersedia
                } - Berat : ${datas[i].berat} gram</p>
${buttonContent}
</div>
</div>
</div>
